...
function CandidateRegister() {
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true);
    try {
      await register('candidate', { name, email, password });
      setToast({ open: true, message: 'Registration successful!', severity: 'success' });
      setTimeout(() => navigate('/candidate/login'), 1000);
    } catch (e) {
      setError(e.message);
    ...
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none' }}>
      <Paper sx={{ p: 5, borderRadius: 4, minWidth: 380, boxShadow: 6, position: 'relative', overflow: 'visible', animation: 'fadeInUp 0.7s cubic-bezier(.4,2,.6,1)'}} elevation={4}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="h5" fontWeight={700} mb={1}>Candidate Register</Typography>
          <Typography variant="body2" color="text.secondary" mb={2}>Create your account to apply for jobs</Typography>
        </Box>
        ...
        <Button onClick={() => navigate('/candidate/login')} sx={{ mt: 2, fontWeight: 600, fontSize: 16, borderRadius: 2, transition: 'background 0.2s', ':hover': { background: 'linear-gradient(90deg, #1976d2 60%, #42a5f5 100%)', color: '#fff' } }} fullWidth>Already have an account? Login</Button>
