...
function HRLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await login('hr', email, password);
      localStorage.setItem('hr', JSON.stringify(data.user));
      localStorage.setItem('hrToken', data.token);
      setToast({ open: true, message: 'Login successful!', severity: 'success' });
      setTimeout(() => navigate('/hr/home'), 1000);
    } catch (e) {
      setError(e.message);
    ...
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none' }}>
      <Paper sx={{ p: 5, borderRadius: 4, minWidth: 380, boxShadow: 6, position: 'relative', overflow: 'visible', animation: 'fadeInUp 0.7s cubic-bezier(.4,2,.6,1)'}} elevation={4}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="h5" fontWeight={700} mb={1}>HR Login</Typography>
          <Typography variant="body2" color="text.secondary" mb={2}>Sign in to manage your jobs and applications</Typography>
        </Box>
        ...
        <Button onClick={() => navigate('/hr/register')} sx={{ mt: 2, fontWeight: 600, fontSize: 16, borderRadius: 2, transition: 'background 0.2s', ':hover': { background: 'linear-gradient(90deg, #1976d2 60%, #42a5f5 100%)', color: '#fff' } }} fullWidth>Don't have an account? Register</Button>
